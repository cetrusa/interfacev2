...
class CargueHistoricoCostos:
            ...
            for index, row in df_resultado.iterrows():
                ...
                try:
                    with self.engine_mysql_bi.connect() as conn:
                        ...
                        conn.execute(sql_upsert, parametros)
                except Exception as e:
                    print(f"Error al insertar/actualizar el registro: {e}")

    # def insertar_o_actualizar(self, df_resultado):
    #     for index, row in df_resultado.iterrows():
    #         # Aquí, necesitas construir tu consulta SQL de inserción o actualización
    #         # basada en la lógica de si ya existe un registro para la fecha, almacén y producto específicos.
    #         # Esta es una plantilla básica para una sentencia UPSERT en MySQL, adaptala según tus necesidades.
    ...
    #         VALUES (%(fechaHora)s, %(almacen)s, %(producto)s, %(costoPromedioInicial)s, %(unidadesInicial)s, %(costoCompradia)s, %(unidadesCompradia)s, %(unidadesMovimientodia)s, %(costoPromedioFinal)s, %(unidadesFinal)s)
    #         ON DUPLICATE KEY UPDATE
    #         costoPromedioInicial=VALUES(costoPromedioInicial), unidadesInicial=VALUES(unidadesInicial), costoCompradia=VALUES(costoCompradia), unidadesCompradia=VALUES(unidadesCompradia), unidadesMovimientodia=VALUES(unidadesMovimientodia), costoPromedioFinal=VALUES(costoPromedioFinal), unidadesFinal=VALUES(unidadesFinal);
    #         """)

    #         # Asegúrate de adaptar los parámetros y la consulta a tus necesidades específicas.
    #         parametros = {
    #             'fechaHora': row['fechaHora'],
    #             'almacen': row['almacen'],
    #             'producto': row['producto'],
    #             'costoPromedioInicial': row['costoPromedioInicial'],
    #             'unidadesInicial': row['unidadesInicial'],
    #             'costoCompradia': row['costoCompradia'],
    #             'unidadesCompradia': row['unidadesCompradia'],
    #             'unidadesMovimientodia': row['unidadesMovimientodia'],
    #             'costoPromedioFinal': row['costoPromedioFinal'],
    #             'unidadesFinal': row['unidadesFinal']
    #         }
    ...
    #         except Exception as e:
    #             print(f"Error al insertar/actualizar el registro: {e}")
    #             # Aquí podrías logear el error o manejarlo según sea necesario

    # def insertar_o_actualizar(self, df_resultado):
    #     if isinstance(df_resultado, pd.Series):
    #         df_resultado = df_resultado.to_frame().T
    #     with self.engine_mysql_bi.connect() as connection:
    #         for index, row in df_resultado.iterrows():
