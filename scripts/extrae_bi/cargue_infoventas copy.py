...
class CargueInfoVentas:
    ...
    def insertar_registros_ignore(self, modelo, data_to_insert):
        ...
        try:
            ...
            session.commit()
            print(f"{len(nuevos_registros)} registros insertados en {modelo.__tablename__}.")
    ...
    def filtrar_nuevos_registros(self, modelo, data_to_insert, session):
        claves_unicas = [columna.name for columna in modelo.__table__.columns if columna.primary_key or columna.unique]
        ...
        for batch in self.chunk_data(data_to_insert, batch_size=50000):
            ...
            if not batch:  # Si el batch está vacío después de filtrar, continúa con el siguiente
                continue
            ...
            existing_records_query = session.query(modelo).filter(
                tuple_(*[getattr(modelo, key) for key in claves_unicas]).in_(
                    [tuple(record[key] for key in claves_unicas) for record in batch]
                )
            ).all()
            existing_records = {self.construct_key(record, claves_unicas) for record in existing_records_query}
            
            # Filtra los registros nuevos
            nuevos_registros = [record for record in batch if self.construct_key(record, claves_unicas) not in existing_records]
    ...
    def construct_key(self, record, key_columns):
        if isinstance(record, dict):
            # 'record' es un diccionario, usa la sintaxis de diccionario para acceder a los valores
            return tuple(record[key] for key in key_columns)
        else:
            # Asume que 'record' es una instancia de objeto, usa 'getattr' para acceder a los atributos
            return tuple(getattr(record, key) for key in key_columns)
    ...
    def marcar_registros_como_procesados(self):
        ...
        session = Session()
        try:
            # Paso 1: Recuperar identificadores de registros
            facturas_items_ids = session.query(
                FactFacturasItems.factura_id,
                FactFacturasItems.producto_id,
                FactFacturasItems.tplinea_id,
                FactFacturasItems.nro_linea,
            ).all()
            notas_credito_items_ids = session.query(
            ...
            # Combinar los identificadores en un solo conjunto para evitar duplicados
            combined_ids = set(facturas_items_ids + notas_credito_items_ids)

            # Paso 2: Actualizar tmp_infoventas
            for fac_numero, cod_producto, tipo, nb_linea in combined_ids:
                # Actualizar registros marcados
                session.execute(
                    update(TmpInfoVentas)
                    .where(
                        TmpInfoVentas.Fac_numero == fac_numero,
                        TmpInfoVentas.Cod_producto == cod_producto,
                        TmpInfoVentas.Tipo == tipo,
                        TmpInfoVentas.nbLinea == nb_linea,
                    )
                    .values(procesado=1)
                )
            session.commit()
    ...
    def eliminar_registros_procesados(self):
        ...
        try:
            ...
            session.query(TmpInfoVentas).filter(TmpInfoVentas.procesado == 1).delete()
    ...
    def procesar_cargue_ventas(self):
        ...
        for fecha in pd.date_range(start=self.IdtReporteIni, end=self.IdtReporteFin):
            ...
            if not df_facturas.empty:
                self.insertar_registros_ignore(FactFacturas, df_facturas.to_dict("records"))
            if not df_notas_credito.empty:
                self.insertar_registros_ignore(FactNotasCredito, df_notas_credito.to_dict("records"))

            # Repite el proceso similar para df_facturas_items y df_notas_credito_items
            df_temporal_items = self.preparar_datos_items_temporales(empresa, fecha_str, fecha_str)
            
            df_facturas_items = df_temporal_items[df_temporal_items["tplinea_id"] == "0"].copy()
            if not df_facturas_items.empty:
                self.insertar_registros_ignore(FactFacturasItems, df_facturas_items.to_dict("records"))
            df_notas_credito_items = df_temporal_items[df_temporal_items["tplinea_id"] == "1"].copy()    
            if not df_notas_credito_items.empty:
                self.insertar_registros_ignore(FactNotasCreditoItems, df_notas_credito_items.to_dict("records"))
