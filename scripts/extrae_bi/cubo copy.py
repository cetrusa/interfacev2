...
class CuboVentas:
    ...
    def generar_nombre_archivo(self):
        self.archivo_cubo_ventas = f"Cubo_de_Ventas_{self.database_name}_de_{self.IdtReporteIni}_a_{self.IdtReporteFin}"
    ...
    def guardar_datos_excel_xlsxwriter2(self, table_name, file_path, hoja,wb):
        """
    ...
    def procesar_hoja(self, hoja,wb):
        try:
            ...
            ext = ".csv" if total_records > 1000000 else ".xlsx"
            self.file_path = os.path.join("media", self.archivo_cubo_ventas_base + ext)

            if ext == ".csv":
                self.guardar_datos_csv(table_name, self.file_path)
            else:
                # Aquí debes ajustar para usar un Workbook que agregue hojas al mismo archivo
                self.guardar_datos_excel_xlsxwriter2(table_name, self.file_path, hoja,wb)
    ...
    def procesar_datos(self):
        ...
        self.archivo_cubo_ventas, self.file_path = self.generar_nombre_archivo()
        ...
        # Crear el libro de trabajo una vez y usarlo para todas las hojas
        wb = Workbook(write_only=True)
        for hoja in self.config["txProcedureExcel"]:
            print(f"Procesando hoja {hoja}")
            if not self.procesar_hoja(hoja, wb):  # Pasar el Workbook aquí
                wb.close()
                return {"success": False, "error_message": f"Error al procesar la hoja {hoja}"}

        # Guardar el libro de trabajo
        wb.save(self.file_path)
        print("Proceso finalizado")
        return {"success": True, "file_path": self.file_path, "file_name": self.archivo_cubo_ventas}
